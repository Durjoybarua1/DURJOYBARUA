...
2. **Upload the files**, keeping the exact folder structure:
   ...
   - `assets/hero.png` inside an `assets` folder
...
- **Hero photo** — replace `assets/hero.png` with a new image of the same filename, or update the `src` path in the `<img>` tag inside `.hero-art`.
